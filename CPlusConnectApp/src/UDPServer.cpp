...
#include "Node.h" // we are allowed to call this here as its not within the .h (since thats where the reference is at)
...
void UDPServer::CheckForPackets() {
    if (!IsServerRunning || sock == INVALID_SOCKET) return;
    ...
    int bytesRead = recvfrom(sock, buffer, sizeof(buffer), 0, (struct sockaddr*)&clientAddr, &addrLen);
